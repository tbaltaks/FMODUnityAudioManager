...
- `void SetLocalParameter(int eventInstanceID, LocalAudioParameter parameter, float value, bool ignoreSeekSpeed = false)`: Sets the value of the given local parameter on the given event instance to the given *new* value. Call with `ignoreSeekSpeed` set to `true` in order to override the parameter's 'Seek Speed' and, instead, cause it's value to update to the new value instantly.
...
- `void SetGlobalParameter(GlobalAudioParameter parameter, float value)`: Sets the value of the given global parameter to the given *new* value. Call with `ignoreSeekSpeed` set to `true` in order to override the parameter's 'Seek Speed' and, instead, cause it's value to update to the new value instantly.
