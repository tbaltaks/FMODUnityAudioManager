...
Each of these elements is public, static, and NOT Monobehaviour so they do not need to exsist on a gameobject in a scene at all to function - Any script from anywhere in the Unity project can access them! Simply call an AudioManager method, passing in any arguments it may require, and away you go!

FMODEvents and FMODParameters are generated scripts to allow for automatic named code values. This means that these scripts are generated upon importing this package and regenerated whenever a change in the FMOD build is detected. In this way, they can be fragile - do not touch these scripts whatsoever. If there are compile errors in your Unity project upon importing this package, these scripts will not generate; in this case, or in any event where the code generation fails, there is a manual generation button in the top bar at Tools -> FMOD Management -> Regenerate FMOD Resources. If this does not work, try removing all compile errors and then restarting the project. 
...
#### Public methods
- 'int StartNewEvent(string eventPath)': Starts playback on an event and returns an int ID for that event instance. Make sure to cache the ID in order to access thst same event instance later, for example to stop the event or to set one of it's local parameters. Pass in one of FMODEvents's fields to play that event. This method is desgined for snapshots and timeline events - for playing one shots I recomend seeing PlayOneShot(). There are a number of overloads for this method that allow spatial attenuation (i.e. attachment to a gameobject) by passing a reference to a gameobject's transform or transform + rigidbody.
- 'void PauseEvent(int eventInstanceID)': Pauses event playback. 
- 'void UnpauseEvent(int eventInstanceID)': Unpauses event playback.
- 'void StopEvent(int eventInstanceID, bool allowFadeout = true)': Stops and releases an event. By default, this method allows fadeout when stopping the event - pass in false to override this behaviour and stop event instantly. BE AWARE; once an event instance has been stopped with this method, it's int ID is obsolete and cannot be used again.
- 'void PlayOneShot(string eventPath)': Plays and releases a oneshot type event. Use this method instead of StartNewEvent() for oneshots to allow FMOD to automatically handle the event instance's lifecycle. There are two separate overloads for this method; one that requires a Vector3 position and will spatially attenuate the event on plaback at that position in world space, ane one that requires a gameobject to attach the oneshot to so the attenuation follows that object on playback.
